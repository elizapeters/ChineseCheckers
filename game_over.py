...
def check_middleTri(board, array, firstVal, secondVal):
    times = 0
    for i in array:
        minFill = 3
        maxfill = 21
...
def game_over(board):
    check = check_top_bottom(board, [0,1,2,3], 6) or check_top_bottom(board, [16,15,14,13], 1)
    another = check or check_middleTri(board, [7, 6, 5, 4], 5, 4) or check_middleTri(board, [9, 10, 11, 12], 3, 2)
    return another