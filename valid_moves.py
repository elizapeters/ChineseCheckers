...
def check_jumps(board, node, player1, player2, moves):
    ...
    if (col + 2 < width and col + 4 < width and 
        board[row][col+2] == player1 or board[row][col+2] == player2) and  (board[row][col+4] == 0):
    ...
    if (row - 1 < length and col + 1 < width and row - 2 < length and col + 2 < width and 
        board[row-1][col+1] == player1 or board[row-1][col+1] == player2) and  (board[row-2][col+2] == 0):
        jumpboard = board.copy()
        #This is so in the recursion there isn't an infinte loop of jumping back and forth
        jumpboard[row][col] = -1
...
def valid_moves(board, node, player1, player2):
    ...
    if col + 2 < width and board[row][col+2] == 0:
        moves.append((row, col+2))
    #Checking if you can move one to the left
    if col - 2 < width and board[row][col-2] == 0:
        moves.append((row, col-2))
    #Checking if you can move up left
    if row - 1 < length and col - 1 < width and board[row-1][col-1] == 0:
